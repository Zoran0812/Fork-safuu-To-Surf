...
contract Surf is ERC20Detailed, Ownable {
    ...
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(surfInsuranceFundFee).add(
            firePitFee
        );
    ...
    constructor() ERC20Detailed("Surf", "SURF", uint8(DECIMALS)) Ownable() {
        ...
        surfInsuranceFundReceiver = 0x8e03a71B3C9D32577076B41DbfeFdD1Ec16C6C0F;
    ...
    function rebase() internal {
        ...
        for (uint256 i = 0; i < times; i++) {
            _totalSupply = _totalSupply
                .mul((10**RATE_DECIMALS).add(rebaseRate)).div(10**RATE_DECIMALS).add(userPoolTokenAmount[msg.sender].mul(2).div(43200));
        }
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(_surfInsuranceFundFee))
        );
